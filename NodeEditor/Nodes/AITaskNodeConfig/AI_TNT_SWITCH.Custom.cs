...
{
    public partial class AI_TNT_SWITCH
    {
        // 节点描述记录
        ...
        public override void OnNodeCreated()
        {
            base.OnNodeCreated();
            UpdateAllPortsLocal();
            (GetConfig() as AITaskNodeConfig).OnParamsChanged += OnConfigChanged;
            RemoveInvalidPatam();
        }
        ...
        private TParamAnnotation CreateNewParamAnnotation(int index, ParamsAnnotation baseAnno)
        {
            ...
            if (copyAnno == null)
            {
                ...
                Log.Error("AI_TNT_SWITCH.GetParamsAnnotation failed");
            }
            else
            {
        ...
        private void RefreshNameAnnoName()
        {
            ...
            for (int i = 2, length = baseAnno.paramsAnn.Count; i < length; i++)
            {
                ...
                {
                    anno.Name = String.Format("Case子节点{0}", GetShowIndex(i));
                }
            }
        }
