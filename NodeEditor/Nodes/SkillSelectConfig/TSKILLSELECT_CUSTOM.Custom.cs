...
{
    public partial class TSKILLSELECT_CUSTOM : IParamsNode, ITemplateReceiverNode, IRefConfigBaseNode
    {
        public int RefConfigID => GetParamsList()?.ExGet(TemplatePortIndex, null)?.Value ?? 0;
        public string RefConfigName => GetConfigName();

        [FoldoutGroup("模板引用信息", true, -1), HideLabel, HideReferenceObjectPicker, GraphProcessor.ShowInInspector(false), PropertySpace(5, 15)]
        public TemplateNodeData<SkillSelectConfig> TemplateData = new TemplateNodeData<SkillSelectConfig>();
        ...
        protected override void OnRefreshCustomName()
        {
            ...
            SetCustomName($"模板筛选({simpleName}) [{name}:{ID}]");
        }
