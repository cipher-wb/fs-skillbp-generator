...
namespace NodeEditor
{
    ...
    public class TCommonParamAnnotation
    {
        #region 
        ...
        [FoldoutGroup("$Title"), LabelText("常用数值名"), DelayedProperty]
        [ReadOnly]
        public string Name;

        [FoldoutGroup("$Title"), LabelText("常用数值枚举"), DelayedProperty]
        [ReadOnly]
        public TableDR.TCommonParamType EnumType;
        ...
        public TCommonParamAnnotation(string name, TableDR.TCommonParamType enumType)
        {
            this.Name = name;
    ...
    public sealed class CommonParamAnnotation : ISetting
    {
        public const string name = "常用数值配置";
        ...
        public List<TCommonParamAnnotation> CommonParamAnnos = new List<TCommonParamAnnotation>();

        public CommonParamAnnotation() { }
        ...
        public void PresetParams()
        {
            ...
            try
            {
                var descs2type = new Dictionary<string, TableDR.TCommonParamType>();
                var typeT = typeof(TableDR.TCommonParamType);
                foreach (var enumValue in Enum.GetValues(typeT))
                {
                    var type = (TableDR.TCommonParamType)enumValue;
                ...
                {
                    var anno = CommonParamAnnos.Find(p => { return p.EnumType == item.Value; });
                    if (anno == null)
                    {
                        // 未找到配置信息,直接New一个
                        var data = new TCommonParamAnnotation(item.Key, item.Value);
                        CommonParamAnnos.Add(data);
                        sbError.AppendLine($"【新增】常用参数名： {data.Title}");
                    }
                    // 刷新属性名
                    else
                    {
        ...
        public void UpdateVDList()
        {
            ...
            TableDR.CustomEnumUtility.VD_TCommonParamEnum_Read.Clear();
            TableDR.CustomEnumUtility.VD_TCommonParamEnum_Write.Clear();

            // 字典查找快一点
            Dictionary<int, TCommonParamAnnotation> commonDic = new Dictionary<int, TCommonParamAnnotation>();
            foreach (var item in CommonParamAnnos)
            {
            ...
            // 按模块顺序添加
            var typeT = typeof(TableDR.TCommonParamType);
            for (int i = 0; i < TCommonParamAnnotation.ModuleAnnos.Count; ++i)
            {
                // 按模块顺序添加
                int index = (i + 1) % TCommonParamAnnotation.ModuleAnnos.Count;
                string matchModuleName = TCommonParamAnnotation.ModuleAnnos[index];
                ...
                foreach (var enumValue in Enum.GetValues(typeT))
                {
                    ...
                    bool isCanRead = true;
                    if (commonDic.TryGetValue((int)type, out TCommonParamAnnotation item))
                    {
                    ...
                    {
                        if (isCanWrite)
                            TableDR.CustomEnumUtility.VD_TCommonParamEnum_Read.Add(desc, type);
                        if (isCanRead)
                            TableDR.CustomEnumUtility.VD_TCommonParamEnum_Write.Add(desc, type);
                    }
                }
            }
        }
