...
namespace GraphProcessor
{
	public static partial class NodeProvider
	{
		public struct PortDescription
		{
            ...
            {
                return $"{nodeType.Name} {portType.Name} {portFieldName} {portIdentifier} {portDisplayName}";
            }
		}
		...
		{
			public Dictionary< string, NodeSpecificToGraph>		nodePerMenuTitle = new Dictionary< string, NodeSpecificToGraph>();
		...
		static void BuildGenericNodeCache()
		{
            ...
            //虚拟节点
            foreach (var nodeType in TypeCache.GetTypesDerivedFrom<BaseVirtualNode>())
            {
                if (!IsNodeAccessibleFromMenu(nodeType))
                    continue;
		...
		static void BuildCacheForNode(Type nodeType, NodeDescriptions targetDescription, BaseGraph graph = null)
		{
			...
			if (attrs != null && attrs.Length > 0)
			{
                ...
                {
                    var key = attr.menuTitle + attr.onlyCompatibleWithGraph?.Name ?? string.Empty;
                    targetDescription.nodePerMenuTitle[key] = new NodeSpecificToGraph
                    {
                        menuTitle = attr.menuTitle,
                        nodeType = nodeType,
                        compatibleWithGraphType = attr.onlyCompatibleWithGraph,
                    };
                }
			}
            ...
            else if (nodeType.IsSubclassOf(typeof(BaseVirtualNode)))
            {
                ProvideVirtualNodeCreationDescription(nodeType, targetDescription);
            }
        }
        ...
        static void ProvideVirtualNodeCreationDescription(Type nodeType, NodeDescriptions targetDescription)
        {
		...
		static MonoScript FindScriptFromClassName(string className)
		{
            if (isUseCustomFind)
            {
                return FindScriptFromClassNameCustom(className);
            }
        ...
        public static IEnumerable<(string path, Type type)>	GetNodeMenuEntries(BaseGraph graph = null)
        {
            ...
            {
                if (node.Value.compatibleWithGraphType == null)
                {
                    yield return (node.Value.menuTitle, node.Value.nodeType);
                }
            }
            ...
            if (graph != null && specificNodeDescriptions.TryGetValue(graph, out var specificNodes))
            {
                ...
                {
                    if (node.Value.compatibleWithGraphType == graphType)
                    {
                        yield return (node.Value.menuTitle, node.Value.nodeType);
                    }
                }
        ...
        /// 获取可创建节点列表
        /// </summary>
        /// <param name="portView">入口节点</param>
        /// <param name="graph"></param>
        /// <returns></returns>
