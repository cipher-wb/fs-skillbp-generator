...
namespace GraphProcessor
{
    ...
    {
        private string GetNodeInfoBoxMessage()
        {
            if (nodeTarget != null)
            {
                return $"\n{nodeTarget.GetNodeAnnotation()}\n";
            }
            return "Error, nodeTarget is null";
        }

        [Sirenix.OdinInspector.HideLabel()]
        //TODO 当前更新模式是只要此BaseNode数据发生变化，就全量更新其对应NodeView的所有数据，后续出现卡顿可以考虑去掉这个特性
        [Sirenix.OdinInspector.OnValueChanged(nameof(UpdateFieldValues), true)]
        ...
        static readonly string baseNodeStyle = "GraphProcessorStyles/BaseNodeView";
        ...
        {
            nodeTarget = node;
            nodeTarget.onNodeCustomNameUpdated += OnNodeCustomNameUpdated;
            this.owner = owner;
        ...
        void InitializeView()
        {
            ...
            //显示隐藏
            this.visible = nodeTarget.isVisible;

            //Custom 自定义的Label（目前用于隐藏子节点计数和日志）
            InitNodeCustom();
        ...
        void SetupRenamableTitle()
        {
            ...
            //descTextField = new TextField { isDelayed = true, multiline = true };
            //descTextField.style.display = DisplayStyle.None;
            //titleLabel.parent.Insert(0, descTextField);
        ...
        public void FocusSelect(bool frameSelection = true)
        {
            if (owner != null)
            {
                owner.ClearSelection();
                owner.AddToSelection(this);
                owner.UpdateNodeInspectorSelection();
                if (frameSelection)
                {
                    owner.FrameSelection();
                }
                else
                {
                    owner.FrameAll();
        ...
        protected virtual void DrawDefaultInspector(bool fromInspector = false)
        {
            ...
            foreach (var field in fields)
            {
                ...
                bool showNodeView = field.GetCustomAttribute<ShowNodeViewAttribute>() != null;
                ...
                bool hasShowAsDrawer = field.GetCustomAttribute(typeof(ShowAsDrawer)) != null;
                if (!showNodeView)
                {
                    ...
                    bool showAsDrawer = !fromInspector && hasShowAsDrawer;
                ...
                showInputDrawer |= hasInputAttribute && hasShowAsDrawer;
                ...
                if (labelTextAttribute != null && !string.IsNullOrEmpty(labelTextAttribute.Text))
                    displayName = labelTextAttribute.Text;
        ...
        {
            foreach (var inputFieldElem in fieldControlsMap[field])
            {
                if (newValue is IList list)
                {
                    ...
                    {
                        var value = list[i];
                        var childElem = Utils.CreateElemByFieldType(value.GetType(), $"参数{i+1}");
                        if (value != null && childElem is TextField textField)
                        {
                            textField.value = value.ToString();
                        }
                        inputFieldElem.Add(childElem);
                    }
                }
                else
                {
                    var notify = inputFieldElem as INotifyValueChanged<T>;
                    if (notify != null)
        ...
        protected SerializedProperty FindSerializedProperty(string fieldName)
        {
            ...
            SerializedProperty serializedProperty = owner.serializedGraph.FindProperty("nodes").GetArrayElementAtIndex(i).FindPropertyRelative(fieldName);
        ...
        protected VisualElement AddControlField(FieldInfo field, string label = null, bool showInputDrawer = false, Action valueChangedCallback = null)
        {
            ...
            ////通过在特性中添加type来创建对应的控件
            var showNodeView = field.GetCustomAttribute<ShowNodeViewAttribute>();
            bool isMultil = field.GetCustomAttribute<UnityEngine.MultilineAttribute>() != null;

            if (showNodeView == null)
                return null;

            VisualElement element = null;

            if (showNodeView.IsBindProperty)
            {
            ...
            {
                element = Utils.CreateElemByFieldType(field.FieldType, showInputDrawer ? "" : label, isMultil);
            }
            ...
            if (showNodeView.IsBindProperty && element is PropertyField elementProperty)
            {
                elementProperty.RegisterValueChangeCallback(e =>
                {
            ...
            if (visibleCondition != null)
            {
                ...
                else
                {
                    ...
                    UpdateOtherFieldValue(field, field.GetValue(nodeTarget));
        ...
        protected virtual void UpdateFieldValues()
        {
            ...
            foreach (var kp in visibleConditions)
            {
                UpdateFieldVisibility(kp.Key, nodeTarget.GetType().GetField(kp.Key, BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Instance).GetValue(nodeTarget));
                NotifyNodeChanged();
            }
        ...
        protected void AddSettingField(FieldInfo field)
        {
            ...
            var elementName = label ?? field.Name;
            if (element != null)
            {
        ...
        {
            if (port.direction == Direction.Output && nodeTarget.isHideChildNodes) return;
        ...
        {
            nodeTarget?.OnSelected();
            base.OnSelected();
        }

        public override void OnUnselected()
        {
            nodeTarget?.OnUnSelected();
            base.OnUnselected();
        }
        ...
        public override void SetPosition(Rect newPos)
        {
            ...
            {
                if (!initializing && !owner.initializing && !owner.IsElementMovingByGroup(nodeTarget.GUID))
                {
                    owner.RegisterCompleteObjectUndo("Moved graph node" + newPos.ToString());
                    var oPos = $"{nodeTarget.GUID}:{GetPosition().position}";
                    var nPos = $"{nodeTarget.GUID}:{newPos.position}";
                    owner.AddUnReDoInfo(DateTime.Now.ToString(), BaseGraphView.URDControlType.URDCT_NodePos, oPos, nPos);
                }

                base.SetPosition(newPos);
                nodeTarget.SetPosition(newPos);
                initializing = false;
            }
        }
        ...
        public override void BuildContextualMenu(ContextualMenuPopulateEvent evt)
        {
            //BuildAlignMenu(evt);
            // TODO
            //evt.menu.AppendAction("Remove Group", (e) => OpenNodeScript(), OpenNodeScriptStatus);
            evt.menu.AppendAction("Debug/Open Node Script", (e) => OpenNodeScript(), OpenNodeScriptStatus);
            evt.menu.AppendAction("Debug/Open Node View Script", (e) => OpenNodeViewScript(), OpenNodeViewScriptStatus);
            evt.menu.AppendAction("Debug/Debug", (e) => ToggleDebug(), DebugStatus);
            if (nodeTarget.unlockable)
                evt.menu.AppendAction((nodeTarget.isLocked ? "Debug/Unlock" : "Debug/Lock"), (e) => ChangeLockStatus(), LockStatus);

            ////显示、隐藏功能
            //if (nodeTarget.isHideChildNodes)
            //{
            //    evt.menu.AppendAction("Show ChildNodes", (e) => OpenChildNodeViews(), OpenNodeViewScriptStatus);
            //}
            //else
            //{
            //    evt.menu.AppendAction("Hide ChildNodes", (e) => HideChildNodeViews(), OpenNodeViewScriptStatus);
            //}
        }
        ...
        void OnNodeCustomNameUpdated(string customName)
        {
            UpdateTitle();
        }

        public void HideChildNodeViews()
        {
            owner?.ShowOrHideNodeChildsNodes(nodeTarget, false);
        }

        public void OpenChildNodeViews()
        {
            owner?.ShowOrHideNodeChildsNodes(nodeTarget, true);
        }
