...
namespace GraphProcessor
{
	...
	[System.Serializable]
	public partial class BaseGraph : Sirenix.OdinInspector.SerializedScriptableObject
	{
		...
		/// <returns></returns>
		[System.NonSerialized/*SerializeField*/, Obsolete("Use BaseGraph.nodes instead")]
        ...
        [System.NonSerialized]
        public Dictionary<string, StickyNote>			stickyNotesPerGUID = new Dictionary<string, StickyNote>();
		...
		void InitializeGraphElements()
		{
            ...
            foreach (var stickyNote in stickyNotes.ToList())
            {
                if (string.IsNullOrEmpty(stickyNote.GUID))
                {
					stickyNote.OnCreated();
                }
                stickyNotesPerGUID[stickyNote.GUID] = stickyNote;
				//stickyNote.Initialize(this);
            }
        ...
        public virtual bool Unload()
        {
            foreach (var node in nodes)
            {
                ExceptionToLog.Call(() => { node.Unload(); });
            }
            return true;
        }

        public virtual bool SaveGraphToDisk()
        {
            SaveRet.Length = 0;
            foreach (var node in nodes)
            {
                ExceptionToLog.Call(() => { node.Save(); });
            }
            return true;
        }
        ...
        public BaseNode AddNode(BaseNode node)
		{
            ...
            if (node.computeOrder < nodes.Count && node.computeOrder > 0)
                nodes.Insert(node.computeOrder, node);
            else
			    nodes.Add(node);
			node.Initialize(this);
		...
		public void Disconnect(string edgeGUID)
		{
            ...
            for (int i = edges.Count - 1; i >= 0; --i)
            {
                ...
                {
                    Debug.LogException(new System.Exception($"[{name}] NodeEditor-edge-check Disconnect failed, {i} / {edges.Count}, GUID:{edgeGUID}"));
                    continue;
                }
                var r = edges[i];
                if (r.GUID == edgeGUID)
            ...
            // bug: The deletion of nested edge leads to incorrect deletion of edge
            //edges.RemoveAll(r =>
            //{
            //    if (r.GUID == edgeGUID)
            //    {
        ...
        {
			stickyNotesPerGUID[note.GUID] = note;
        ...
        {
            stickyNotesPerGUID.Remove(note.GUID);
		...
		{
#if UNITY_EDITOR
            // performance optimization modified by wmt
...
#pragma warning restore CS0618
		...
		void DestroyBrokenGraphElements()
		{
			edges.RemoveAll(e =>
            {
                var result = e.inputNode == null
                || e.outputNode == null
                || string.IsNullOrEmpty(e.outputFieldName)
                || string.IsNullOrEmpty(e.inputFieldName);
                if (result == true)
                {
                    Debug.LogError($"【NodeEditor】[{name}] 清理失效连线: inputNode:{e.inputNode?.GetCustomName() ?? string.Empty} : outputNode:{e.outputNode?.GetCustomName() ?? string.Empty}");
                }
                return result;
            });
			nodes.RemoveAll(n => n == null);
