...
// We keep slow checks inside the editor
        ...
        public void GetNodeAndEdgeByOutport(List<BaseNode> nodes, List<SerializableEdge> edges)
        {
            GetEdges()?.ForEach(edge =>
            {
                ...
                if (nextNode != default)
                {
                    ...
                    nextNode.outputPorts.ForEach(port =>
                    {
                        port.GetNodeAndEdgeByOutport(nodes, edges);
                    });
                }
            });
        }

        public SerializableEdge GetInputNodeEdge(BaseNode node)
        {
	...
	public abstract class NodePortContainer : List< NodePort >
	{
		...
		public void Add(SerializableEdge edge)
		{
            ...
            if (port == default && portFieldName == "PackedParamsOutput" && portIdentifier == "0")
            {
                if (node.GetType().Name.Contains("TEVAT_TALK")
                    || node.GetType().Name.Contains("TEVAT_DIALOG")
                    || node.GetType().Name.Contains("TEVAT_ROLE_DIALOG")
                    || node.GetType().Name.Contains("TEVAT_STORYBEGIN")
                    || node.GetType().Name.Contains("TEVAT_STORYEND"))
                {
                    port = node.GetNpcTalkGroupIDPort();
                }
                else if (node.GetType().Name.Contains("TEVAT_CREATE_MODEL"))
                {
                    port = node.GetModelIDPort();
                }
            }
