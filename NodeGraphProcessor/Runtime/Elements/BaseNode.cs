...
namespace GraphProcessor
{
    ...
    [InfoBox("@GetNodeInfoBoxMessage()", InfoMessageType.Warning)]
    public abstract partial class BaseNode
	{
        ...
        /// <summary>
        /// 作为父节点隐藏，我是父节点，隐藏的是子节点
        /// </summary>
        public virtual bool isHideChildNodes => hideChildNodes ;
		...
		/// 节点被引用
		/// </summary>
	    public bool HasBeRef => RefCount > 0;
        ...
        /// <summary>
        /// Triggered after nodeCustomName is updated
        /// </summary>
        public event Action<string>                     onNodeCustomNameUpdated;
        ...
        public virtual void OnNodeRecreated() { }
		...
		void InitializeInOutDatas()
		{
			...
			foreach (var field in fields)
			{
                ...
                var labelTextAttribute = field.GetCustomAttribute<Sirenix.OdinInspector.LabelTextAttribute>();
                ...
                if (!String.IsNullOrEmpty(labelTextAttribute?.Text))
        ...
        public virtual bool CanExport() { return true/*GetAllEdges().Count() > 0*/; }
        ...
        public void Save()
        {
            ...
            ExceptionToLog.Call(() => OnSave());
        }

        public void Load()
        {
            ExceptionToLog.Call(() => OnLoad());
        }
        public void Unload()
        {
            ExceptionToLog.Call(() => OnUnload());
        }
        ...
        /// sync port datas
        /// </summary>
        public void SyncPortDatas()
        {
            // 优先更新，避免迭代内修改port数据异常
            UpdateAllPortsLocal();
        ...
        public NodePort GetNpcTalkGroupIDPort()
        {
            return inputPorts.Concat(outputPorts).FirstOrDefault(p => 
            {
                return p.portData.identifier == "NpcTalkGroupID";
            });
        }

        public NodePort GetModelIDPort()
        {
            return inputPorts.Concat(outputPorts).FirstOrDefault(p =>
            {
                return p.portData.identifier == "ModelID";
            });
        }
		...
		public IEnumerable<SerializableEdge> GetAllEdges()
		{
            // TODO 版本问题，看是否可以删除
            // 迭代内存在变动情况，这边先临时处理下
            var allPosrts = GetAllPorts().ToList();
            foreach (var port in allPosrts)
            {
                var edges = port.GetEdges().ToList();
        ...
        /// <summary>
        /// Return input the connected edges of the node
        /// </summary>
        /// <returns></returns>
        ...
        /// <summary>
        /// Return output the connected edges of the node
        /// </summary>
        /// <returns></returns>
        ...
        {
            bool update = nodeCustomName != customName;
            nodeCustomName = customName;
            if (update)
            {
                onNodeCustomNameUpdated?.Invoke(customName);
            }
        }
        ...
        public virtual void GetChildNodesRecursive(List<BaseNode> childNodes, bool isIncludeNodeWithoutEdge = false)
        {
            foreach (var outport in outputPorts)
            {
                foreach (var edge in outport.GetEdges())
                {
                    ...
                    inputNode.GetChildNodesRecursive(childNodes);
                }

                // check break Edge's config
                if (isIncludeNodeWithoutEdge)
                {
                    // TODO
                }
            }
        ...
        public void GetChildNodesRecursive<TNode>(List<BaseNode> childNodes)
        {
            foreach (var outport in outputPorts)
            {
                foreach (var edge in outport.GetEdges())
                {
                    ...
                    if(inputNode != null)
                    {
                        if (inputNode is TNode)
                        {
                            childNodes.Add(inputNode);
                        }
                        inputNode.GetChildNodesRecursive<TNode>(childNodes);
                    }
                }
            }
        }
        ...
        /// <summary>
        /// 获取当前节点指定outport后指定的node
        /// </summary>
        /// <typeparam name="TOutPortStart"></typeparam>
        /// <typeparam name="TNode"></typeparam>
        /// <param name="childNodes"></param>
        public void GetChildNodesRecursive<TOutPortStart, TNode>(List<BaseNode> childNodes)
        {
            foreach (var outport in outputPorts)
            {
                if(outport.portData.displayType == typeof(TOutPortStart))
                {
                    foreach (var edge in outport.GetEdges())
                    {
                        var inputNode = edge.inputNode;
                        if(inputNode != default)
                        {
                            if (inputNode is TNode)
                            {
                                childNodes.Add(inputNode);
        ...
        public virtual void GetParentNodes(List<BaseNode> parentNodes)
        {
        ...
        /// </summary>
        /// <returns></returns>
        public virtual BaseNode GetReferenceNode()
        {
            return default;
        }
        ...
        /// <returns></returns>
        public T GetPreviousNode<T>() where T : BaseNode
        {
            if(inputPorts == default || inputPorts.Count == 0) { return default; }

            var inputEdges = inputPorts[0].GetEdges();
            if(inputEdges?.Count == 0) { return default; }

            var inputEdge = inputEdges[0];
            var previousNode = inputEdge.outputNode;
            return previousNode as T;
        }
        ...
        /// <typeparam name="T"></typeparam>
        /// <returns></returns>
        public T GetLoopPreviousNode<T>() where T : BaseNode
        {
            var previousNode = GetPreviousNode<BaseNode>();
            while (true)
            {
                if(previousNode == default)
                {
                    return default;
                }
                else if (previousNode is T tNode)
                {
                    return tNode;
                }
                else
                {
                    previousNode = previousNode.GetPreviousNode<BaseNode>();
                }
            }
        }
        ...
        /// <typeparam name="T"></typeparam>
        /// <returns></returns>
        public List<T> GetNextNodes<T>(Type outportType) where T : BaseNode
        {
            if (outputPorts == default || outputPorts.Count == 0) { return default; }

            var nodes = new List<T>();
            foreach(var port in outputPorts)
            {
                if (port.portData.displayType == outportType)
                {
                    var edges = port.GetEdges();
                    if(edges == default || edges.Count == 0) { continue; }

                    foreach(var edge in edges)
                    {
                        var nextNode = edge.inputNode;
                        if (nextNode is T tNode)
                        {
                            nodes.Add(tNode);
                        }
                    }
        ...
        /// </summary>
        /// <returns></returns>
        public bool IsLastNodeInParent()
        {
            if (inputPorts == default || inputPorts.Count == 0) { return false; }

            var inputEdges = inputPorts[0].GetEdges();
            if (inputEdges?.Count == 0) { return false; }

            var inputEdge = inputEdges[0];
            var edges = inputEdge.outputPort.GetEdges();
            if(edges?.Count == 0) { return false; };

            var lastNode = edges.Last()?.inputNode;
        ...
        /// </summary>
        /// <returns></returns>
        public List<SerializableEdge> GetSiblingEdge()
        {
            if (inputPorts == default || inputPorts.Count == 0) { return default; }
            ...
            var inputEdge = inputEdges[0];
            return inputEdge.outputPort.GetEdges();
        }
        ...
        /// </summary>
        /// <returns></returns>
        public int GetInParentNodeIndex()
        {
            if (inputPorts == default || inputPorts.Count == 0) { return -1; }

            var inputEdges = inputPorts[0].GetEdges();
            if (inputEdges?.Count == 0) { return -1; }

            var inputEdge = inputEdges[0];
            var edges = inputEdge.outputPort.GetEdges();
            if (edges?.Count == 0) { return -1; };

            return edges.FindIndex( edge =>
            {
                return edge.inputNode == this;
            });
        }

        public IEnumerable<T> GetParentNodeAllChildren<T>() where T : BaseNode
        {
            if (inputPorts == default || inputPorts.Count == 0) { yield break; }
            ...
            if(inputEdges?.Count > 0)
            {
                ...
                if(edges?.Count > 0)
                {
                    foreach (var edge in edges)
                    {
                        yield return edge.inputNode as T;
                    }
                }
            }
        }
        ...
        #region 调试断点
        public class NodeDebugBreakpointData
        {
            public int ParamIndex;
            public bool Enable;
            public int OpType;    // 1==  2!=  3>  4>=  5<  6<=
            public int CValue;

            public NodeDebugBreakpointData(int paramIndex)
            {
            ...
            public void SetOpType(string opStr)
            {
                switch(opStr)
                {
                    case "==":
        ...
        [NonSerialized]
        private Dictionary<int, NodeDebugBreakpointData> nodeDebugBreakpointDic = new Dictionary<int, NodeDebugBreakpointData>();

        public NodeDebugBreakpointData GetBreakpointData(int paramIndex)
        {
            if (nodeDebugBreakpointDic.ContainsKey(paramIndex))
                return nodeDebugBreakpointDic[paramIndex];
            else
                return null;
        }

        public NodeDebugBreakpointData GetAndCretteBreakpointData(int paramIndex)
        {
            if (!nodeDebugBreakpointDic.ContainsKey(paramIndex))
            {
                NodeDebugBreakpointData bpData = new NodeDebugBreakpointData(paramIndex);
                nodeDebugBreakpointDic.Add(paramIndex, bpData);
            }
            return nodeDebugBreakpointDic[paramIndex];
        }

        public Dictionary<int, NodeDebugBreakpointData> GetDebugBreakpointDic()
        {
            return nodeDebugBreakpointDic;
        }
        #endregion
    }
}
